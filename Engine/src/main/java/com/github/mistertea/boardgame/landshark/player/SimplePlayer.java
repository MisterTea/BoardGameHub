...
public class SimplePlayer extends AbstractPlayer {
	...
	public String fetchCommand(StateQuery query, State state) {
		...
		System.out.println("FETCHING COMMAND FOR PLAYER " + name + " WITH STATE " + state);
		...
		case AUCTION: {
		  if (state.auctionState.bids.containsKey(name)) {
		    // I've already bid, wait for the auction to end.
		    return null;
		  }
			...
			int maximumBidGivenRisk = Math.max(
					0,
					myState.cash
							- query.estimateRentBeforeStart(state, myState, 0.95f,
									100));
		...
		case BUYING_HOUSES: {
			...
			if (query.canBuildHouses(state)) {
				int maximumRisk = Math.max(
						0,
						myState.cash
								- query.estimateRentBeforeStart(state, myState,
										0.95f, 100));
				int totalCost = 0;
				for (String propertyName : myState.properties) {
					...
					// houses,
					// try to diversify
					if (query.canPutHouses(state, myState, propertyName, 1)) {
